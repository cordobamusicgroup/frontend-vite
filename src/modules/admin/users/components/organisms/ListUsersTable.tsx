...
const ListUserTable: React.FC<Props> = ({ setNotification }) => {
  ...
  const handleDelete = async (id: number): Promise<void> => {
    await deleteUsers.mutateAsync([id], {
      ...
      onError: (error: any) => {
        setNotification({ message: `Error deleting user: ${formatError(error).message.join(', ')}`, type: 'error' });
      },
    });
  };
  ...
  const handleResendEmail = async (email: string): Promise<void> => {
    await resendWelcomeEmail.mutateAsync(email, {
      ...
      onError: (error: any) => {
        setNotification({ message: `Error sending email: ${formatError(error).message.join(', ')}`, type: 'error' });
      },
    });
  };
