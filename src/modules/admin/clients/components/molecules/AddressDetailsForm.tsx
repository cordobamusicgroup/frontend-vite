...
const AddressDetailsForm: React.FC = () => {
  ...
  return (
    <>
      ...
      <Autocomplete options={countries || []} getOptionLabel={(option) => option.name} loading={countriesLoading} onChange={(event, value) => setValue("address.countryId", value ? value.id : null)} value={selectedCountry} isOptionEqualToValue={(option, value) => option.id === value.id} renderInput={(params) => <TextFieldForm {...params} required name="address.countryId" label="Country" />} />
