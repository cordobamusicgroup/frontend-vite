...
const ClientSchema = z.object({
  clientId: z.number().readonly(), // read
...
export const ClientValidationSchema = BaseSchema.superRefine((data, ctx) => {
  ...
  if (data.contract.status === "TERMINATED") {
    if (!data.contract.endDate) {
      ...
      if (data.contract.startDate && dayjs.isDayjs(data.contract.startDate) && dayjs.isDayjs(data.contract.endDate) && !data.contract.endDate.isAfter(data.contract.startDate)) {
        ctx.addIssue({
          path: ["contract", "endDate"],
          code: z.ZodIssueCode.custom,
